...
export default function AnimatedGradient({
  ...
  // Eski sürümdeki `isMounted` state'i kaldırıldı: useEffect zaten yalnızca
  // istemcide çalışır, state bir tam render turu bekletip WebGL kurulumunu
  // geciktiriyordu — ilk yükleme performansı için buradan kazanıldı.
  useEffect(() => {
    ...
    // prefers-reduced-motion: tek kare çiz, döngü kurma. globals.css'teki
    // `.animate-*` disipliniyle aynı sözleşme — hareket azaltma isteyen
    // kullanıcıya sonsuz WebGL animasyonu oynatmak onu ihlal ederdi.
    ...
    const animate = (time: number) => {
      ...
      // İlk kare çizildi → canvas görünür olsun. Canvas opacity 0 başlar ve
      // CSS transition ile açılır: WebGL hazır olana dek altındaki statik
      // katman görünür, hazır olunca yumuşak geçiş olur (ani sıçrama yok).
...
function hexToRgba(hex: string): [number, number, number, number] {
  ...
  // `noUncheckedIndexedAccess` altında dizi/dizin erişimi `undefined`
  // dönebilir; eksik parça 0 sayılır (orijinal NaN üretirdi, bu daha iyi).
  ...
  if (hex.startsWith("rgba(")) {
    ...
    a = isHsla ? num(parts[3]) : 1;
    [r, g, b] = hslToRgb(h, s, l);
  } else if (hex.startsWith("#")) {
    const c = hex.slice(1);
    if (c.length === 3) {
      r = parseInt((c[0] ?? "0").repeat(2), 16) / 255;
      g = parseInt((c[1] ?? "0").repeat(2), 16) / 255;
      b = parseInt((c[2] ?? "0").repeat(2), 16) / 255;
    } else if (c.length >= 6) {
